...
Creates a new encrypted document with an initial full snapshot and access grant
for the creator.
...
Appends an incremental encrypted patch and atomically updates the latest
encrypted snapshot. Each edit includes an Ed25519 signature and resulting
...
HMAC-SHA256 — verify using the `X-Webhook-Signature` header.
...
on creation — store it securely. Verify payloads by comparing
