...
export function useDocuments(identity: StoredIdentity | null) {
  ...
  const queryId = identity?.id;

  const { data: liveData, isLoading: dbLoading } = db.useQuery(queryId ? {
    accessGrants: {
...
export function useDocumentEdits(
  ...
  const editsQueryId = documentId;

  const { data: liveData } = db.useQuery(editsQueryId ? {
    documents: {
